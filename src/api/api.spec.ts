...
import { fetchUsersByUserName, fetchReposByName } from './api';
import { ApiResponse } from './api.types';

it('fetch user data from github search', async () => {
  mockAxios.get.mockImplementationOnce(() =>
    Promise.resolve<AxiosResponse<ApiResponse<User>>>({
      config: {},
      headers: {
  ...
  const response = await fetchUsersByUserName('anis');
  const users = (await response.data.items) as User[];
  expect(
...
it('fetch repo data from github search', async () => {
  mockAxios.get.mockImplementationOnce(() =>
    Promise.resolve<AxiosResponse<ApiResponse<Repository>>>({
      config: {},
      headers: {
        Accept: 'application/vnd.github.v3+json',
      },
  ...
  const response = await fetchReposByName('theme');
  const users = (await response.data.items) as Repository[];
  expect(
